...
df = df[df["Price"] != 0]
df["year"] = df["Product"].apply(
    lambda x: x.split()[-1]
    if x.split()[-1].isnumeric() and int(x.split()[-1]) >= 2000
    else x
)
...
df["year"] = df["year"].apply(
    lambda x: x.split()[1] if x.split()[0].lower() == "used" else x
)
df["year"] = df["year"].apply(
    lambda x: x if x.isnumeric() and len(x) == 4 else "Unknown"
)
...
clean_product["Brand"] = clean_product["Product"].apply(
    lambda x: x.split()[1]
...
including_brand = including_brand.reset_index()[
    ["Product", "Price", "year", "City", "Brand"]
]
